...
def _(estaciones_l1, linea1):
    ...
    estaciones_l1_1 = estaciones_l1.to_crs('EPSG:4326')
    linea1_1 = linea1.to_crs('EPSG:4326')
    # ¿Cómo se ven las estaciones?
    estaciones_l1_1
    return estaciones_l1_1, linea1_1
...
@app.cell
def _(estaciones_l1_1, linea1_1, plt):
    _fig, _ax = plt.subplots(figsize=(20, 10))
    linea1_1.plot(color='blue', ax=_ax, alpha=0.4)
    nombre_estaciones = estaciones_l1_1['NOMBRE'].tolist()
    estaciones_l1_1.plot(categorical=True, column='NOMBRE', categories=nombre_estaciones, cmap='Blues', edgecolor='black', legend_kwds={'bbox_to_anchor': (1.3, 1)}, legend=True, ax=_ax)
    _fig.tight_layout()
    return
...
def _(estaciones, lineas, plt):
    ...
    # Diccionario de colores: Uno para cada línea
        ...
        if 'NOMBRE' in lineas[linea_key].columns:
            ...
            linea_col = 'Name'
        if 'NOMBRE' in estaciones[estacion_key].columns:  # print(linea_key, estaciones_key)
            estacion_col = 'NOMBRE'
        else:
            estacion_col = 'Name'
        ...
        for point_idx, point in enumerate(points):
            ...
            ax.annotate(estacion_name, (x, y), fontsize=fontsize, rotation=angle, arrowprops=dict(color=colors[index]))
...
def _(caminos, plt):
    ...
    caminos['Camino_2012'].plot(categorical=True, column='TIPOIITEJ', legend=True, ax=_ax)
    plt.show()
    return
